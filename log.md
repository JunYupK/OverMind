...
- **브랜치:** master (PR #1 하네스, PR #2 스킬 벤더링 모두 머지됨)
...
- **`settings.gradle.kts`가 log.md 가드의 감시 경로 밖이다.** 이번 감시 경로 확장에도
  포함되지 않았다 — `build.gradle.kts`는 보면서 빌드 설정의 나머지 절반은 안 본다
...
### 2026-09-02 · Claude Code (원격 세션) · 이월 결함 HEAD 등재 · claude/overmind-handover-8njuet

- **한 일:** 인수인계 대화에만 존재하던 이월 결함 7건을 HEAD 블록에 등재했다.
  같은 세션에서 HEAD 갱신을 따로 준비했지만 `feat/widen-log-guard`(PR #3)가 같은 일을
  더 넓게 하고 있어서 그쪽을 기준으로 삼고 내 HEAD 재작성은 버렸다. 이 브랜치는 PR #3 위로
  리베이스했고 남은 것은 겹치지 않는 델타뿐이다
- **결과:** `guardrails` 통과(5건), `test`(L1) 20건 통과. **`verify`는 완주하지 못했다** —
  이 원격 컨테이너에 Docker가 없어 L2(Testcontainers)를 돌릴 수 없다. CI가 실제 판정이다
- **함정:** 두 세션이 같은 HEAD 블록을 동시에 덮어쓰면 반드시 충돌한다. HEAD는 append가
  아니라 overwrite로 정의돼 있어서 **병렬 세션에 대해 구조적으로 취약하다** — 세션 기록은
  append-only라 충돌하지 않는데 HEAD만 그렇다. 지금은 사용자 1명이라 감당되지만, 도구를
  병렬로 돌리기 시작하면 "누가 HEAD를 소유하는가"를 정해야 한다.
  둘째: **인수인계 문서의 환경 지침은 로컬 Windows 기준이고 원격 세션에는 적용되지 않는다.**
  여기서는 `JAVA_HOME` export도 MSYS 경로 변환도 필요 없다 — Java 21이 이미 PATH에 있다.
  대신 Docker가 없어 L2가 통째로 빠지므로 `./gradlew verify`는 "통과"가 아니라 "실행 불가"다.
  둘을 같은 말로 적으면 다음 세션이 L2를 검증된 것으로 오해한다. gitleaks도 없어 로컬
  `guardrails`는 그 단계를 건너뛴다
