...
- **마일스톤:** **M0 — Task 2 구현·검증 완료, Task 3 진행 예정**
- **최근 갱신:** 2026-09-03 · Codex (Task 2 완료)
- **브랜치:** `codex/m0-t2-t3` (`origin/master`의 `ecc4fb6`에서 시작)
- **verify:** L1 51건 / L2 1건 통과(로컬 Docker) · **guardrails:** 11건 통과. gitleaks 미설치로 로컬 시크릿 스캔 생략

### 진행 중

- **Task 2~3만 구현한 뒤 커밋·푸시·PR 생성까지 수행한다.** 사용자가 새 브랜치에서의
  작업과 마무리를 승인했다. Task 3의 V2 마이그레이션도 이 범위에 포함된다.
- **M0 설계·플랜 확정, Task 0~1 병합 완료, Task 2 구현·검증 완료.**
...
1. Task 3 V2 스키마·JPA 매핑을 구현한다.
   두 게이트를 통과한 변경을 커밋·푸시하고 master 대상 PR을 연다. Task 4와 PR 병합은 범위 밖이다.
