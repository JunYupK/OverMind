...
- **최근 갱신:** 2026-09-11 · Claude Code (원격 세션) — **PR #15 Codex 리뷰 대응.**
  세 건 전부 실재했다. ① **D7을 통과로 적은 것이 틀렸다.** 박스 밖에서
  `X-Forwarded-Host: evil.example`가 `resource`에 반영되지 않는 것을 보고 채웠는데,
  Tomcat이 보는 피어 주소는 **항상 Docker 브리지 게이트웨이**(실측 `172.20.0.1`)이고
  그것은 `internal-proxies` 안이다. 헤더가 앱까지 갔다면 반영됐어야 하므로, 반영되지
  않은 것은 **Caddy가 덮어썼다**는 뜻이다 — 그 curl은 Caddy의 정규화를 증명했지
  `internal-proxies`를 증명하지 않았고, 정규식을 `.*`로 바꿔도 통과한다. 게다가 앱이
  루프백에만 게시돼 있어 **신뢰 범위 밖 피어가 Tomcat에 도달할 경로 자체가 없다.**
  이 경계의 실제 보호막은 D1의 호스트 포트 바인딩이다. 칸을 비우고, 거부 동작은 수동
  스모크가 아니라 **L2 테스트로 옮긴다**고 적었다(M1 과제).
  ② 스펙 §8.5가 여전히 "지금 DCR로 가는 것은 타당하다", 플랜 H1이 여전히 "DCR 활성화"를
  지시하고 있었다 — D-Q와 정면 충돌이고, 그대로 따르면 D-Q가 막으려는 실패를 재현한다.
  둘 다 정정했다. ③ 스모크 5를 채웠는데 그 전제인 2번이 비어 있었다 — 멱등성 자체는
  관측했으나 대상이 2번의 USER가 아니라 PROJECT였다. 칸을 되돌리고 사유를 적었다.
  **"게이트가 통과했다"와 "게이트가 무언가를 막았다"를 혼동한 사례가 오늘 또 나왔다** —
  이번에는 내가 채운 표에서 나왔다.
...
- **최근 갱신:** 2026-09-10 · Codex — **루트 README 초안 작성.** 현재 M0의
  ...
  중복하지 않고 `deploy/README.md`, 실제 완료 판정은 `70-m0-smoke.md`로 연결했다.
  `test` 159/159와 `guardrails` 11/11은 통과했다. Docker daemon을 띄울 권한이
  없어 Testcontainers L2 82건이 초기화 실패해 `verify`는 완료하지 못했다.
...
### 2026-09-10 05:17 · Codex · README 초안 · 커밋 예정

- **한 일:** 두 줄뿐이던 루트 README를 M0 기준 프로젝트 소개, 기능·범위, 빠른 시작,
  MCP 연결, 보안, 배포, 백업·복구, 개발 테스트, 구조, 설계 문서, 로드맵을 담은 초안으로
  확장했다. 상세 운영 명령은 기존 `deploy/README.md`를 단일 진실의 원천으로 유지했다.
- **결과:** 문서 링크와 `git diff --check` 확인, `test` 159/159와 `guardrails` 11/11
  통과. Docker 설치 후 daemon 기동도 시도했지만 컨테이너 권한상 네트워크 설정이
  거부되어 Testcontainers L2 82건이 초기화 실패했고 `verify`는 완료하지 못했다.
- **함정:** 로컬에는 `master` ref나 `origin/master`가 없고 현재 `work` HEAD가 PR #13의
  병합 결과다. README는 이 현재 HEAD의 구현 범위를 기준으로 썼다.
- **다음:** 사용자 리뷰 후 문구를 다듬고, 실제 배포에서는 수동 스모크 D1~D13을 채운다.
