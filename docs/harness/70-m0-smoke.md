...
| 5 | 같은 `idempotency_key`로 2번을 다시 호출 | `created: false`, 같은 `observation_id`. **2번이 아직 비어 있어 이 항목을 채우지 않는다** — 2026-09-10에 멱등성 자체는 관측했으나 대상이 2번의 USER observation이 아니라 3번 계열의 PROJECT observation이었다(`created:false`, 같은 `observation_id` 확인). 2번을 먼저 채운 뒤 그 키로 다시 확인한다 | | |
...
| D7 | forwarded 헤더 신뢰 경계 | **이 항목은 이 토폴로지에서 수동으로 검증할 수 없다 — 2026-09-10에 그것이 밝혀졌다.** 그날 박스 밖에서 `X-Forwarded-Host: evil.example`를 보내 `resource`에 반영되지 않는 것을 확인하고 통과로 적었으나, Codex 리뷰가 그 논증이 성립하지 않음을 지적했고 확인 결과 맞았다. 근거: Tomcat이 보는 피어 주소는 **항상 Docker 브리지 게이트웨이**(실측 `172.20.0.1`)이고 그것은 `internal-proxies` 안, 즉 신뢰 대상이다. 따라서 그 헤더가 앱까지 도달했다면 앱은 **반영했어야** 한다. 반영되지 않았다는 것은 **Caddy가 덮어썼다**는 뜻이며, 그 curl이 증명한 것은 Caddy의 헤더 정규화지 `internal-proxies`가 아니다 — 정규식을 `.*`로 바꿔도 똑같이 통과한다. 그리고 앱 포트가 루프백에만 게시되어 있어 **신뢰 범위 밖의 피어가 Tomcat에 도달할 경로 자체가 없다.** 이 경계의 실제 보호막은 `internal-proxies`가 아니라 **D1의 호스트 포트 바인딩**이다. `internal-proxies` 자체의 거부 동작은 수동 스모크가 아니라 **L2 테스트로 옮겨야 한다**(신뢰 범위 밖 피어 주소를 가장해 `RemoteIpValve`가 헤더를 무시하는지 단언) — M1 과제. 그때까지 이 칸은 비워 둔다 | | |
